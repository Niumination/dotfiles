...
stateMonitor.directive('stateConnected', function () {
    return {
        ...
        controller: ['$scope', 'ratingStore', function ($scope, ratingStore) {
            chrome.storage.local.get(["enableCount", "runCount"], function (e) {
                $scope.dontRequestRating = (parseInt(e.runCount) < 3 &&
                    parseInt(e.enableCount) < 10) ||
                    enabled === 'false';
                $scope.ratingStore = ratingStore;
                //console.log(e.runCount, e.enableCount, enabled);
            });
        }]
    };
});
